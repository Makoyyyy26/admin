    ...
    loginForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (data.success && data.otp_required) {
          // Credentials OK — redirect to OTP verification immediately
          Swal.fire({
            icon: 'success',
          ...
          setTimeout(() => { window.location.href = "otp.php"; }, 1300);
        } else if (data.success) {
          window.location.href = "dashboard.php";
        } else {
          Swal.fire({ icon: 'error', title: 'Login Failed', text: data.message || 'Invalid username or password', confirmButtonColor: '#059669' });
          signInBtn.disabled = false;
          signInBtn.textContent = "Sign In";
        }
      } catch (err) {
        Swal.fire({ icon: 'error', title: 'Connection Error', text: 'Connection error. Please try again.', confirmButtonColor: '#059669' });
        signInBtn.disabled = false;
        signInBtn.textContent = "Sign In";
      }
    });
